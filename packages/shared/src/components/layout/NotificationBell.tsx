...
const NOTIFICATION_ICONS: Record<NotificationType, React.ReactNode> = {
  appointment_reminder: <Calendar className="h-4 w-4 text-amber-600" />,
  ...
  system: <Settings className="h-4 w-4 text-stone-500" />,
...
function NotificationItem({ notification, onMarkRead }: {
  ...
  return (
    <div
      className={`flex items-start gap-3 px-4 py-3 hover:bg-stone-50 transition-colors cursor-pointer ${
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm ${!notification.read ? 'font-medium text-stone-800' : 'text-stone-600'}`}>
          {notification.title}
        </p>
...
export function NotificationBell() {
  ...
  return (
    <div className="relative" ref={panelRef}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 max-h-96 bg-white rounded-xl border border-stone-200 shadow-lg overflow-hidden z-50">
          {/* Header */}
          ...
          {/* List */}
          <div className="overflow-y-auto max-h-72 divide-y divide-stone-100">
            {loading ? (
