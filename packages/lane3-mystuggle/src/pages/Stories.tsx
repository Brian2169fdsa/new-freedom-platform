    ...
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      ...
      <div className="flex items-center gap-3 p-4 pb-2">
        <div className="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center">
          {story.isAnonymous ? (
            <BookHeart className="h-5 w-5 text-blue-700" />
          ) : (
      ...
      <div className="flex items-center gap-1 px-4 py-2 border-t border-slate-100">
        ...
        <button className="text-slate-400 hover:text-blue-600 p-1.5">
  ...
  if (!open) {
    return (
      <button
        ...
        className="w-full flex items-center gap-3 bg-white rounded-xl border border-slate-200 p-4 text-left hover:border-blue-300 transition-colors"
      >
        <div className="h-10 w-10 rounded-full bg-blue-50 flex items-center justify-center flex-shrink-0">
          <PenSquare className="h-5 w-5 text-blue-600" />
        </div>
        <span className="text-sm text-slate-400">Share your story...</span>
      </button>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-blue-200 p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-semibold text-slate-800 text-sm">Write Your Story</h3>
      ...
      {/* Writing Prompts */}
      <div className="mb-3">
        <p className="text-xs text-slate-500 mb-2">Need inspiration? Try a prompt:</p>
      ...
      <textarea
        ...
        className="w-full resize-none bg-slate-50 rounded-lg border border-slate-200 p-3 text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-300 min-h-[160px]"
        rows={6}
      />
      ...
      <div className="flex items-center justify-between mt-3">
        <button
          ...
          className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg transition-colors ${
            isAnonymous ? 'bg-blue-100 text-blue-700' : 'text-slate-400 hover:text-slate-600 bg-slate-100'
          }`}
        >
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-1.5 bg-blue-500 text-white text-sm font-medium rounded-lg hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {posting ? 'Publishing...' : (
            <>
...
export default function Stories() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-bold text-slate-800">Community Stories</h1>
        <p className="text-sm text-slate-500 mt-0.5">Real stories from real people. You are not alone.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-8 text-center">
          <BookHeart className="h-10 w-10 text-slate-300 mx-auto" />
          <h3 className="font-medium text-slate-800 mt-3">No stories yet</h3>
        ...
        <div className="space-y-4">
          <p className="text-xs text-slate-400">{stories.length} stories shared</p>
          {stories.map((story) => (
