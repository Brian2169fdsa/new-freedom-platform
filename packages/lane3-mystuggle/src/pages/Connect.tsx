      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-stone-800 text-sm">Available Mentor</h3>
          ...
          <button
            onClick={onRequest}
            className="mt-3 flex items-center gap-1.5 px-4 py-1.5 bg-amber-500 text-white text-xs font-medium rounded-lg hover:bg-amber-600 transition-colors"
          >
  ...
  return (
    <div className="bg-white rounded-xl border border-stone-200 p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h3 className="font-medium text-stone-800 text-sm">Mentor Match</h3>
            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[match.status] || statusColors.ended}`}>
              {match.status.charAt(0).toUpperCase() + match.status.slice(1)}
            </span>
          </div>
          <p className="text-xs text-stone-500 mt-1">{match.matchReason}</p>
          ...
          {match.status === 'active' && (
            <button className="mt-2 flex items-center gap-1.5 text-amber-600 text-xs font-medium hover:text-amber-700">
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-stone-500 truncate mt-0.5">{conversation.lastMessage}</p>
        </div>
        {unread > 0 && (
          <span className="h-5 w-5 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center flex-shrink-0">
            {unread}
          </span>
        )}
      </div>
...
export default function Connect() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-bold text-stone-800">Connect</h1>
        <p className="text-sm text-stone-500 mt-0.5">Find a mentor or connect with peers</p>
      </div>
      ...
      {activeTab === 'find' && (
        <div className="space-y-3">
          <div className="bg-gradient-to-br from-amber-50 to-stone-50 rounded-xl border border-amber-200 p-4">
            <h3 className="font-semibold text-stone-800 text-sm mb-1">How Mentoring Works</h3>
            <div className="space-y-2 mt-3">
              {[
                ...
                <div key={step} className="flex items-center gap-3">
                  <div className="h-6 w-6 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center text-xs font-bold flex-shrink-0">
                    {step}
                  </div>
