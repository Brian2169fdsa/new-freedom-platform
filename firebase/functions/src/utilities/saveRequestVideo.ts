...
/**
 * Saves RequestMediaElement (video, plus a thumbnail) to Firebase
 ...
 */
export const saveRequestVideo = async (
  requestMediaElement: RequestMediaElement,
  rootDir: string,
  userId: string): Promise<MediaElement> => {
  // Save incoming request media item's video.
  ...
  const originalURL = await saveVideo(originalFilepath, originalData);

  // Save incoming thumbnail related to video.
